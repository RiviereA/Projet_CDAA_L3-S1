...
namespace Projet2CDAA
{
    public class LesProduits
    {
        ...
        /// <summary>
        /// Méthode recherchant un produit dans la liste des produist à partir de son nom
        /// </summary>
        /// <param name="name">Nom du produit à chercher</param>
        /// <returns>Renvoie le produit correspondant</returns>
        public Produit GetProduitNom(string name)
        {
            Produit prod = null;
            foreach (Produit p in listProd)
                if (p.Nom == name) prod = p;
            return prod;
        }
        ...
        public List<Produit> ListePAR()
        {
            ...
            foreach (Produit p in listProd)
            {
                if ((p.ToString().Contains("Type de risque : ")) && (p.ToString().Contains("Regles : "))) liste.Add(p);
            }
