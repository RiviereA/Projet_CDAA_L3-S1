...
namespace Projet2CDAA
{
    public class LesMagasins
    {
        ...
        /// <summary>
        /// Méthode recherchant un magasin dans la liste des magasins à partir de son nom
        /// </summary>
        /// <param name="name">Nom du magasin à chercher</param>
        /// <returns>Renvoie le magasin correspondant</returns>
        public Magasin GetMagasinNom(string name)
        {
            Magasin mag = null;
            foreach (Magasin m in listMag)
                if (m.Nom == name) mag = m;
